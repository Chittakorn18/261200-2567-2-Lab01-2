...
public class Turtle implements Runnable, ActionListener, MouseListener, MouseMotionListener, KeyListener, ComponentListener, MouseWheelListener
{
    ...
    public static void main(String[] a)
    {
        ...
        for(int i=0;i<360;i++) {
            ...
            bob.left(1);}
        bob.up();
        bob.left(180); // move 30 units (pen down = draw a line)       // turn left for 90 degrees
        bob.down();           // pick pen (tail) up      // move 40 units (pen up = no drawing)
        for(int i=0;i<360;i++) {
            bob.forward(1.5);
            bob.width(2);
            bob.left(1);} // change pen color
        bob.down();
        bob.up();         // put pen (tail) down (so it's ready to draw)
        bob.home();
        bob.left(90);
        bob.penColor("black");
        bob.forward(90);
